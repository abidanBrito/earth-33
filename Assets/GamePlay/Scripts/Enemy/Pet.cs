...
public class Pet : BaseGame
{
        ...
        //si no hay ningun enemigo controlado por el jugador lo hace directamente
                if(!BaseGame._HasPet){
                    if(!_ControllingEnemy){
                                ...
                                BaseGame._sphereControlling = _sphere;     // Poniendo la esfera en las constantes para tener en cuenta cual es la que esta siendo utilizada para controlar
                                ...
                                BaseGame._HasPet = true;                   // Poniendo que el jugador tiene mascota
                                BaseGame.Pet = gameObject;                 // La mascota es el transform
                    ...
                    if(BaseGame.Pet != gameObject){
                        if(!_ControllingEnemy){
                                Destroy(BaseGame.Pet.gameObject);
                                BaseGame._sphereControlling._Movements = -1; // Poniendo la esfera anterior a que vuelva a su posicion
                                BaseGame._sphereControlling = _sphere;       // Poniendo la nueva esfera como la actual controlando el enemigo
                                BaseGame._usingBall = false;               // Poniendo el estado de usando esferas a falso para que las demas puedan ser utilizadas
                                _sphere._Movements = 1;                         // La nueva esfera pasa a estar en estado controlando
                                _ControllingEnemy = true;                       // Enemigo controlado
                                BaseGame._HasPet = true;                   // Pet = true
                                BaseGame.Pet = gameObject;                 // La nueva mascota es el nuevo tranform
                                gameObject.layer = 11;  //   Layer: Pet         // Poniendo en layer Pet para que los enemigos puedan encontrar la mascota y atacarla
    ...
    {
                BaseGame._sphereControlling._Movements =-1; // La esfera vuelve a su posicion
                BaseGame._sphereControlling._Mode = -1;     // Mientras vuelve se le quita el modo 
                BaseGame._sphereControlling = null;         // Global = null
                _ControllingEnemy = false;                       
                BaseGame._HasPet = false;
                BaseGame.Pet = null;
    ...
    void Update()
    {
        if(_EnemyControlled){
            ...
            if(BaseGame._sphereControlling._Movements == 1){
                BaseGame._sphereControlling.transform.position = _BallPosition.position;
                //BaseGame._sphereControlling.transform.rotation = _BallPosition.transform.rotation;
            }
